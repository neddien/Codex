...
namespace codex {
    ...
    void AssetManager::init(fs::VirtualFilesystem& vfs, std::string_view asset_root) noexcept
    {
        auto& self      = get();
        self.vfs_       = &vfs;
        self.root_path_ = asset_root;
        self.registry_  = Box<AssetRegistry>::make(*self.vfs_, self.root_path_);
    ...
    Shared<IAssetLoader> AssetManager::loader_by_ext(const std::string& extension) noexcept
    {
        ...
        if (extension[0] == '.') {
            ...
            if (auto it = self.loaders_by_ext_.find(util::crypto::fnv1a(nodotext)); it != self.loaders_by_ext_.end())
                return it->second;
        }
    ...
    Shared<IAssetLoader> AssetManager::loader_by_type(const std::string& type_name) noexcept
    {
        ...
        if (auto it = self.loaders_by_type_.find(util::crypto::fnv1a(type_name)); it != self.loaders_by_type_.end())
            return it->second;
        return nullptr;
    }
                                         ...
                                         const IAssetImportSettings* params) noexcept
    {
        ...
        // Technically vfs_ and root_path_ should never change but just in case
        fs::VirtualFilesystem* vfs{};
        Shared<IAssetLoader>   loader{};
        ...
        auto fh = vfs->open((stdfs::path{ root_path } / meta.storage_path).generic_string());
        if (!fh)
            return nullptr;
        ...
        if (loader) {
            return loader->load_asset(std::move(fh), meta.import_settings.get());
        } else
