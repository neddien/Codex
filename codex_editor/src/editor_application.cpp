    ...
    void EditorApplication::on_init()
    {
        ...
        if (!stdfs::exists(s_var_app_data_path_)) {
            try {
                ...
                Engine::warn("Failed to create application data folder!");
            }
        }

        Engine::info("Application data path: '{}'", s_app_data_path_.string());
        Engine::info("Variable application data path: '{}'", s_var_app_data_path_.string());
