                            ...
                            if (it == behaviours.end()) {
                                if (ImGui::Selectable(script.c_str(), false)) {
                                    ...
                                    if (script_instance) {
                                        ...
                                        Engine::error("Failed to create an NB instance of: {}", script);
                                    }
                                }
                            }
                        }
                        ...
                        if (ImGui::BeginCombo("##texture_filter_mode", preview_item)) {
                            if (ImGui::Selectable("Nearest", props.filter_mode == opengl::TextureFilterMode::Nearest)) {
                                if (props.filter_mode != opengl::TextureFilterMode::Nearest) {
                                    ...
                                    texture->create(texture->file_path(), new_props);
                                }
                            }
                            ...
                            if (ImGui::Button("Load texture", { 100, 0 })) {
                                ...
                                if (NFD_OpenDialogU8(&outPath, filters, 1, nullptr) == NFD_OKAY) {
                                    std::filesystem::path relative_path =
                                        std::filesystem::relative(outPath, std::filesystem::current_path());
                                    NFD_FreePathU8(outPath);
                                    c.sprite.set_texture(Resources::load<gfx::Texture2D>(relative_path));
                                }
                            }
                            ImGui::EndGroup();
                            ImGui::Columns(1);
