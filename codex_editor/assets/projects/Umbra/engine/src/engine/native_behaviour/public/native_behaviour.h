...
namespace codex {
    ...
    public:
        virtual ~NativeBehaviour() { detail::nbman_logger().Info("~NativeBehaviour()"); };
    ...
    protected:
        ...
        void log(const lgx::Level level, const std::string_view fmt, TArgs&&... args) const
        {
            detail::nbman_logger().Log(level, fmt, std::forward<TArgs>(args)...);
        }
        template <typename... TArgs>
        void info(const std::string_view fmt, TArgs&&... args) const
        {
            detail::nbman_logger().Info(fmt, std::forward<TArgs>(args)...);
        }
        template <typename... TArgs>
        void warn(const std::string_view fmt, TArgs&&... args) const
        {
            detail::nbman_logger().Warn(fmt, std::forward<TArgs>(args)...);
        }
        template <typename... TArgs>
        void error(const std::string_view fmt, TArgs&&... args) const
        {
            detail::nbman_logger().Error(fmt, std::forward<TArgs>(args)...);
        }
        template <typename... TArgs>
        void fatal(const std::string_view fmt, TArgs&&... args) const
        {
            detail::nbman_logger().Fatal(fmt, std::forward<TArgs>(args)...);
        }
        template <typename... TArgs>
        void trace(TraceLocation fmt_spec, TArgs&&... args) const
        {
            ...
            detail::nbman_logger().Log(lgx::Level::Debug, msg);
        }
