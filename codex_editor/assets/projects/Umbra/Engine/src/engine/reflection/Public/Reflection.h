...
namespace codex::rf {
    ...
    public:
        ...
        inline void add_property(const std::string_view name, const PropertyType type, const usize offset,
                                 const std::string_view displayName = "", const std::string_view category = "General")
        {
            properties_.emplace_back(name, type, offset, displayName, category);
        }
        ...
        template <typename T>
        T* property_value(object obj, const std::string_view propertyName) const noexcept
        {
            const Property* prop = find_property(propertyName);
            if (!prop)
                return nullptr;
