...
namespace glm {
    namespace detail {
        ...
        //	{
        //		return ((floatTo11bit(x) & ((1 << 11) - 1)) << 0) |  ((floatTo11bit(y) & ((1 << 11) - 1)) << 11) |
        //((floatTo10bit(z) & ((1 << 10) - 1)) << 22);
        //	}
    ...
    GLM_FUNC_QUALIFIER vec<4, T, Q> packRGBM(vec<3, T, Q> const& rgb)
    {
        ...
        T                  Alpha =
            clamp(max(max(Color.x, Color.y), max(Color.z, static_cast<T>(1e-6))), static_cast<T>(0), static_cast<T>(1));
