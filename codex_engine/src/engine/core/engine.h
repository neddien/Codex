...
namespace codex {
    ...
    public:
        [[nodiscard]] static auto engine_logger() noexcept -> lgx::Logger& { return detail::engine_logger(); }
        [[nodiscard]] static auto editor_logger() noexcept -> lgx::Logger& { return detail::editor_logger(); }
        [[nodiscard]] static auto nbman_logger() noexcept -> lgx::Logger& { return detail::nbman_logger(); }

        static void use_engine_logger() noexcept { detail::set_active_logger(detail::engine_logger()); }
        static void use_editor_logger() noexcept { detail::set_active_logger(detail::editor_logger()); }
        static void use_nbman_logger()  noexcept { detail::set_active_logger(detail::nbman_logger()); }

        template <typename... TArgs>
        static void log(const lgx::Level level, const std::string_view fmt, TArgs&&... args)
        {
            detail::active_logger().Log(level, fmt, std::forward<TArgs>(args)...);
        }
        template <typename... TArgs>
        static void info(const std::string_view fmt, TArgs&&... args)
        {
            detail::active_logger().Info(fmt, std::forward<TArgs>(args)...);
        }
        template <typename... TArgs>
        static void warn(const std::string_view fmt, TArgs&&... args)
        {
            detail::active_logger().Warn(fmt, std::forward<TArgs>(args)...);
        }
        template <typename... TArgs>
        static void error(const std::string_view fmt, TArgs&&... args)
        {
            detail::active_logger().Error(fmt, std::forward<TArgs>(args)...);
        }
        template <typename... TArgs>
        static void fatal(const std::string_view fmt, TArgs&&... args)
        {
            detail::active_logger().Fatal(fmt, std::forward<TArgs>(args)...);
        }
        template <typename... TArgs>
        static void trace(TraceLocation fmt_spec, TArgs&&... args)
        {
