...
namespace codex::gfx {
    ...
    public:
        [[nodiscard]] static Shader*                   shader() noexcept;
    ...
    public:
        static void init(std::filesystem::path shader_path);
        static void dispose();
    ...
    public:
        ...
        {
            render_rect(sprite.texture().get(), sprite.texture_coords(), transform, sprite.colour(), sprite.z_index(),
                        entity_id);
        }
