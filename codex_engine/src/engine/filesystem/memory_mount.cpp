...
namespace codex::fs {
    ...
    Shared<FileHandle> MemoryMount::open(const std::string& path, const FileProperties props) noexcept
    {
        ...
        if (file_it == files_.end()) {
            ...
            return Shared<MemoryFileHandle>::make(npath, it->second.buffer, it->second.mutex, shared_from_this(),
                                                  props);
        }
        ...
        auto mfh = Shared<MemoryFileHandle>::make(npath, entry.buffer, entry.mutex, shared_from_this(), props);
