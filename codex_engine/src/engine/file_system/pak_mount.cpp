...
namespace codex::fs {
    ...
    {
    public:
        PakFileHandle(const std::string_view path, mem::Shared<FileHandle> handle, const PakEntry entry,
                      mem::Shared<PakMount> owner, const FileProperties props = FileProperties{})
    ...
    private:
        ...
        mem::Shared<PakMount>     owner_;
        mem::Shared<FileHandle>   handle_;
    };

    PakMount::PakMount(mem::Shared<FileHandle> handle, const i32 priority)
        : handle_{ std::move(handle) }
        , priority_{ priority }
    {
        if (handle_) {
    ...
    mem::Shared<FileHandle> PakMount::open(const std::string& path, const FileProperties props) noexcept
    {
        const auto npath = normalize(path);
        const u64  hash  = util::crypto::djb2_hash(npath);
        auto       it    = std::lower_bound(entries_.begin(), entries_.end(), hash,
        ...
        while (it != entries_.end() && it->path_hash == hash) {
            if (std::string_view{ it->path } == npath) {
                if (props.mode == FileMode::Read)
                    return mem::Shared<PakFileHandle>::make(npath, handle_, *it, new_shared_from_this(), props);
                return nullptr;
            }
            ++it;
        }
