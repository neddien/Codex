...
namespace codex::fs {
    ...
    mem::Shared<FileHandle> DiskMount::open(const std::string& path, const FileProperties props) noexcept
    {
        ...
        return mem::Shared<LinuxFileHandle>::make(fd, normalize(path), props);
        ...
        return mem::Shared<NTFileHandle>::make(handle, normalize(path), props);
