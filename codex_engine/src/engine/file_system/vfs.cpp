...
namespace codex::fs {
    ...
    VFS::VFS(mem::Shared<IVFSMount> default_mount) noexcept
    {
        root_         = mem::Box<Node>::make();
        root_->dir    = true;
        root_->path   = "/";
        root_->mounts = {};
        ...
        if (default_mount) {
            ...
            mount(mem::Shared<MemoryMount>::make(0), "/");
        }
    }

    bool VFS::mount(mem::Shared<IVFSMount> mount, const std::string& path, const bool mk) noexcept
    {
        ...
        auto pos = std::lower_bound(node->mounts.begin(), node->mounts.end(), mount,
                                    [](const mem::Shared<IVFSMount>& a, const mem::Shared<IVFSMount>& b)
    ...
    bool VFS::unmount(const std::string& path, mem::Shared<IVFSMount> mount) noexcept
    {
        std::scoped_lock lock{ mutex_ };
        ...
        auto it = std::find_if(node->mounts.begin(), node->mounts.end(),
                               [&mount](const mem::Shared<IVFSMount>& m) { return m.get() == mount.get(); });
    ...
    mem::Shared<FileHandle> VFS::open(const std::string& path, const FileProperties props) noexcept
    {
        const auto npath       = normalize(path);
    ...
    bool VFS::exists(const std::string& path) const noexcept
    {
        std::shared_lock lock{ mutex_ };
    ...
    bool VFS::mkdir_nolock(const std::string& path, const bool recursive) noexcept
    {
        const auto npath      = normalize(path);
        auto       components = util::str::split(npath, '/');
        auto*      cur        = root_.get();
        ...
        for (auto it = components.begin(); it != components.end(); ++it, ++depth) {
            ...
            if (auto dir_it = cur->children.find(c); dir_it != cur->children.end()) {
                ...
                if (std::prev(components.end()) == it || recursive) {
                    ...
                    auto [ins, _] = cur->children.emplace(c, mem::Box<Node>::make(std::move(node)));
                    cur           = ins->second.get();
                } else {
                    return false;
                }
    ...
    bool VFS::mkdir(const std::string& path, const bool recursive) noexcept
    {
        std::scoped_lock lock{ mutex_ };
        return mkdir_nolock(path, recursive);
    }

    std::vector<std::string> VFS::list_nolock(const std::string& dir) const noexcept
    {
    ...
    bool VFS::is_directory_nolock(const std::string& path) const noexcept
    {
        auto  components  = util::str::split(normalize(path), '/');
        auto* cur         = root_.get();
        auto* mount_node  = root_.get();
    ...
    std::vector<std::string> VFS::list(const std::string& dir, const ListOptions opts) const noexcept
    {
        ...
        if (!recursive) {
            ...
            for (auto& name : entries) {
                const std::string full   = dir.empty() ? name : normalize(dir) + '/' + name;
        ...
        stk.push({ normalize(dir), "" });
        while (!stk.empty()) {
            auto [cur_dir, prefix] = std::move(stk.top());
            stk.pop();
            for (auto& name : list_nolock(cur_dir)) {
                const std::string full        = cur_dir.empty() ? name : cur_dir + '/' + name;
    ...
    bool VFS::is_directory(const std::string& path) const noexcept
    {
        std::shared_lock lock{ mutex_ };
        return is_directory_nolock(path);
    }

    cc::Task<mem::Shared<FileHandle>> VFS::open_async(std::string path, FileProperties props) noexcept
    {
        co_await Engine::get_worker_pool();
        co_return open(path, props);
    }

    cc::Task<bool> VFS::exists_async(std::string path) const noexcept
    {
        co_await Engine::get_worker_pool();
        co_return exists(path);
    }

    cc::Task<bool> VFS::mkdir_async(std::string path, bool recursive) noexcept
    {
        co_await Engine::get_worker_pool();
        co_return mkdir(path, recursive);
    }

    cc::Task<std::vector<std::string>> VFS::list_async(std::string dir) const noexcept
    {
        co_await Engine::get_worker_pool();
        co_return list(dir);
    }

    cc::Task<bool> VFS::is_directory_async(std::string path) const noexcept
    {
        co_await Engine::get_worker_pool();
        co_return is_directory(path);
    }

    bool VFS::export_to_pak(mem::Shared<FileHandle> out, const PakProperties props) noexcept
    {
        ...
        while (!node_st.empty()) {
            ...
            for (auto& mount : cur->mounts) {
                for (const auto& list_entry : mount->list("/", ListOptions::FilesOnly | ListOptions::Recursive)) {
                    ...
                    if (rel_path == std::string_view{ out->path() }.substr(
                                        std::string_view{ out->path() }.find_first_not_of('/')))
                        continue;
                    ...
                    if (rel_path.size() >= sizeof(PakEntry::path)) {
                        error("Cooker: Path too long for PAK entry (max {} chars): {}",
                              sizeof(PakEntry::path) - 1, full_path);
                        return false;
                    }
    ...
    VFS::Node* VFS::walk_to(const std::string& path, Node** const previous_node) noexcept
    {
        const auto npath = normalize(path);
        Node*      cur   = root_.get();
