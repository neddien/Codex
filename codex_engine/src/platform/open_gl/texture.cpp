...
namespace codex::opengl {
    Texture::Texture(std::filesystem::path file_path, TextureProperties properties)
        : file_path_(std::move(file_path))
        , props_(properties)
    {
        const auto str_path = file_path_.generic_string();
        ...
        if (!buffer_) {
            throw std::runtime_error(
                "[OpenGL]::[ERROR] >> Could not open texture file for reading: " + file_path.string());
        }
