...
- [x] SDL audio disabled due to Fedora package issues (using FMOD instead)

- [ ] Some warnings disabled in debug builds

- [x] CMake install targets reference hardcoded `SDL2` instead of conditional `SDL2-static` on Windows (line 411 in codex_engine/CMakeLists.txt)

- [x] `BOX2D_BUILD_TESTBED` is set twice in codex_engine/CMakeLists.txt (lines 7-8 and 10-11)

- [x] FMOD paths use `${CMAKE_CURRENT_DIR}` which should be `${CMAKE_CURRENT_SOURCE_DIR}` (line 364)
...
| **Asset Manager** | PARTIAL | Core types, loader registration, registry scan + .meta generation implemented; no caching, no prefabs |
| **Prefabs** | MISSING | No entity prefab/template system; `PrefabFile` enum defined but unimplemented |
...
#### 2. Asset Manager (In Progress)
- Core types implemented: `IAsset`, `IAssetLoader`, `AssetLoaderBase<T, TSettings>`, `AssetPath`, `Asset<T>`, `CX_ASSET()` macro
- `AssetManager` singleton: `load<T>(fh)`, `load<T>(fh, params)`, `register_loader<TLoader>(exts)`, `get_loader_by_ext()`, `get_loader_by_type()`
- `AssetRegistry::scan(vfs, path)` implemented: async directory scan, CRC32 checksumming, `.cxmeta` sidecar generation
- `AssetMetadata` serialization to/from JSON `.meta` files
- `Texture2DLoader` registered for `png/jpg/jpeg/bmp/gif` — loader stub (returns empty, full load not yet wired)
- **Missing**: asset caching/ref-counting, UUID resolution public API, `unload()`, hot-reload, ContentBrowserView integration
...
3. **Asset Manager** (IN PROGRESS) - Wire `Texture2DLoader`, add caching, UUID resolution
4. **Prefabs + Entity Cloning** - Foundation for spawning; `Entity::Clone()` required
